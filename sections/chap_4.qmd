...
The first propery of $\R$ we investigate is the Archimedean property. This is on of the most useful properties of $\R$, and it essentially states that the set of natural numbers $\N$ is not bounded above in $\R$. 
...
::: Remark
The Archimedean property might sound trivial. However it far from trivial.
Indeed there are examples of ordered fields $K$ that satisfy:
...
In particular (@eq-theorem-nested-proof-1) is saying that it should hold
$$
...
$x$ should be defined as a supremum. Define the set
$$
...
$${#eq-example-nested}
Since $x>0$, by the Archimedean Property in Theorem \ref{theorem-archimedean} Point 1, there exists $n_0 \in \N$ such that
$$
0 < \frac{1}{n_0} < x \,.
$$
...
The first property is an alternative characterization of the supremum, which we will often use.
...
\inf A = a \,,
$$
since the proof of $\sup A = b$ is similar. 

By definition of $A$, we have that $a$ is a lower bound for $A$, since 
$$
a < x \,, \quad \forall \, x \in A \,.
$$
We need to prove that $a$ is the largest lower bound of $A$. To this end, let $L$ be a lower bound for $A$, that is,
$$
...
The result we discuss below is so important that it deserves its own section: We are going to
show that $\Q$ is dense in $\R$. This means that for every given number $x \in \R$, there exists
a rational number $r \in \Q$ which is arbitrarily close to $x$. In other words, $\Q$ and 
$\R$ are tightly knitted together: For every pair of $a,b \in \R$ with $a<b$, there exists $r \in \Q$ such that
$$
a < r < b \,.
$$
...
How many gaps are there in $\Q$? In other words, how many irrational numbers are out there?
