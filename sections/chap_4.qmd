...
Let $S \subset \R$. We say that $S$ is dense in $\R$ if for every $x \in \R$ and $\e \in \R$ with $\e>0$, there exist $q \in \Q$ such that 
$$
|x - q| < \e \,.
$$
...
::: Remark
Let $S \subset \R$. They are equivalent: 

- $S$ is dense in $\R$.
- For every pair of numbers $x,y \in \R$ with $x<y$, there exists $q \in \Q$ such that
$$
x < q < y \,.
$$
...
Let $X$ be a countable set and $A \subset X$. Then either $A$ is finite or countable.
...
g(k) := f(n_k) \,.
$$
In this way we have defined a function $g \colon \N \to A$. As $g$ was 
defined thourgh $f$, and $f$ is injective, we have that $g$ is injective. Moreover $g$ is surjective by construction. Hence $g$ is bijective, showing that $A$ is countable.
...
4. Let $X = \Z$ the set of integers. Define 
$$
f(n):= 
\begin{cases}
