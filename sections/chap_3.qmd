...
- Consider an arbitrary set $A \subset K$ such that $A \neq \emptyset$ and $A$ is bounded above.
...
In particular the above tells us that $m$ is not an upper bound for $A$, given that $\tilde{a} \in A$ and $m < \tilde{a}$. Therefore $m \in B^c$, by definition of $B^c$. Therefore(@eq-equiv-cut-complete-2) implies
$$
m \leq s \,,
$$
...
Let us start with the definition of ${\N}_{\R}$. We have the intuition that ${\N}_{\R}$ should be
$$
...
\mathbf{1}:=1\,,
$$
with $1$ the neutral element of the multiplication in $\R$, which exists by the field axiom (M2). We could then define $\mathbf{2}$ by setting
$$
\mathbf{2} := 1 + 1 \,.
$$
...
::: Definition
### Inductive set
Let $S \subset \R$. We say that $S$ is an inductive set if they are satisfied:
...
::: Theorem  
### ${\N}_{\R}$ is the smallest inductive subset of $\R$   {#theorem-smallest-inductive}

Let $C \subset \R$ be an inductive subset. Then
$$
{\N}_{\R} \subset C \,.
$$
...
Let $\mathcal{M}$ be the collection of **all** inductive subsets of $\R$. Therefore, since
$C$ is an inductive subset, we have
$$
C \cap \bigcap_{M \in \mathcal{M}} M  = \bigcap_{M \in \mathcal{M}} M \,.
$$
By definition of ${\N}_{\R}$ and the above observation we get
$$
C \cap {\N}_{\R} = C \cap \bigcap_{M \in \mathcal{M}} M  = \bigcap_{M \in \mathcal{M}} M  = {\N}_{\R} \,.
$$
Now, by definition of intersection of sets, we have
$$
({\N}_{\R} \cap C) \subset C \,.
$$
Since 
$$
({\N}_{\R} \cap C) = {\N}_{\R}\,,
$$
we conclude that ${\N}_{\R} \subset C$.
:::
...
By Theorem \ref{theorem-smallest-inductive} we conclude that
$$
{\N}_{\R} \subset C \,,
$$
...
Another consequence of the definition of ${\N}_{\R}$ and the field axioms is the 
...
Let $\alpha(x)$ be a statement depending on $n \in {\N}_{\R}$. Assume that 
...
{\N}_{\R} \subset C\,.
$$
As by definition $C \subset {\N}_{\R}$, we have proven that 
$$
{\N}_{\R} = C \,,
$$
