...
3. An element $e \in K$ is called **neutral element** of $\circ$ if
    $$ 
    x \circ e = e \circ x = e \,, \quad \, \forall \, x \in K
    $$
...
- If $L \in A$, then by definition we will have
$$
L < \sqrt{2} \,.
$$
Now the idea is that $\sqrt{2} \notin \Q$, and hence we can always find some $\tilde{L} \in \Q$ such that 
$$
L < \tilde{L} < \sqrt{2} \,.
$$ {#eq-theorem-Q-cut-2}
Therefore $\tilde{L} \in A$, and (@eq-theorem-Q-cut) implies 
$$
\tilde{L} \leq L \,,
$$
which contradicts (@eq-theorem-Q-cut-2).

- If $L \in B$, then by definition we will have
$$
 \sqrt{2} < L \,.
$$
Similarly, as $\sqrt{2} \notin \Q$, we can find $\tilde{L} \in \Q$ such that 
$$
\sqrt{2} < \tilde{L} < L \,.
$$
Arguing as above we find a contradiction.

Therefore both $L \in A$ and $L \in B$ lead to a contradiction. We conclude that $(A,B)$ has no separator in $\Q$.
:::
