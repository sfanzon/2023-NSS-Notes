...
We need to prove that $K$ is complete. To this end, consider $A \subset K$ non-empty and bounded above. Define the set of upper bounds of $A$:
$$
B := \{ b \in K \divider b \geq a \,, \,\, \forall a \in A \} \,.
$$
We have $B \neq \emptyset$, since $A$ is bounded above. 
...
*Proof of Claim*.  
$(B^c,B)$ forms a partition of $K$, since $B^c \neq \emptyset$. Thus $B \neq \emptyset$ and 
$$
...
x \leq \tilde{a} \leq y \,,
$$
concluding (@eq-equiv-cut-complete-1) and the claim.

Since $(B^c,B)$ is a cut, by the Cut Propery there exists a separator $s \in K$ such that 
$$
...
s < m < \tilde{a} \,.
$$
In particular the above tells us that $m$ is not an upper bound for $A$, given that $\tilde{a} \in A$. Therefore $m \in B^c$. By (@eq-equiv-cut-complete-2) we then have
$$
m \leq s \,,
$$
which contradicts $s < m$. Therefore $s$ is an upper bound of $A$.
...
$$
Recalling that $B$ is the set of upper bounds of $A$, this means that $s$ is the least upper bound of $A$, that is, 
$$
s = \sup A \in K \,.
$$
...
$$
$$
a \leq b \,, \quad \forall a \in A \,, \,\, b \in B \,.
$${#eq-equiv-cut-complete-3}
Since $B \neq \emptyset$, from (@eq-equiv-cut-complete-3) it follows that $A$ is bounded above: just take any element $b \in B$ in estimate (@eq-equiv-cut-complete-3) to see it. Since $A \neq \emptyset$, by Completeness we have
$$
s = \sup A \in K \,.
$$
In particular 
...
The answer is **YES**, and there are several models for such $\R$. If time allows, we will look into one of these models at the end of the module.
