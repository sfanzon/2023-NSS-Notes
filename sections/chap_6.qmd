...
Definition \ref{definition-bounded-sequence} says that a sequence is bounded, if we can find some constant $M$ (which might be big, it only has to exist), such that for all elements of the sequence it holds that 
$$
...
Proof. Suppose the sequence $\left(a_{n}\right)_{n \in \mathbb{N}}$ converges. Let

$$
a:=\lim _{n \rightarrow \infty} a_{n}
$$

This limit exists, since we assumed it converges. Hence, we have that

$$
\forall \varepsilon>0: \exists n_{0} \in \mathbb{N}: \forall n \geq n_{0}:\left|a_{n}-a\right| \leq \varepsilon
$$

In particular, we can take $\varepsilon=1$ and let $n_{0}$ be that value such that, for all $n \geq n_{0}$, we have $\left|a_{n}-a\right| \leq 1$. Then we have for all $n \geq n_{0}$ that, by the triangle inequality,

$$
\left|a_{n}\right|=\left|a_{n}-a+a\right| \leq\left|a_{n}-a\right|+|a| \leq 1+|a| \text {. }
$$

If we choose

$$
M:=\max \left\{\left|a_{1}\right|,\left|a_{2}\right|, \ldots,\left|a_{n_{0}-1}\right|, 1+|a|\right\}
$$

(note that this maximum exists since we take the maximum of a finite number of elements) then we indeed have that for all $n \in \mathbb{N},\left|a_{n}\right| \leq M$.

This proof basically says that the sequence can behave wildly in the beginning, but only up to a finite number $n_{0}$. After that it will stay close to the value of the limit, we is assumed to exist.

Example 2.2.10. In Theorem 2.2.2 we show that the sequence $\left(\frac{1}{n}\right)_{n \in \mathbb{N}}$ converges (to 0). Hence, it follows from Theorem 2.2.9 that this sequence is bounded.

Indeed, since $n \geq 1$ for all $n \in \mathbb{N}$, we have that, for all $n \in \mathbb{N}$,

$$
\left|\frac{1}{n}\right|=\frac{1}{n} \leq 1
$$

Remark. Note that this does not work in general for functions. You could for example say that for the function $f:(0, \infty) \rightarrow \mathbb{R}, x \mapsto \frac{1}{x}$ we have that $f(x) \rightarrow 0$ as $x \rightarrow \infty$, but $\frac{1}{x}$ is not bounded since it can get arbitrarily large for $x$ close to 0 . For sequences we do not have this problem, since $a_{n}$ is only specified for $n \in \mathbb{N}$.

Taking the contrapositive of the statement in Theorem 2.2.9 we get the following corollary:

Corollary 2.2.11. If a sequence $\left(a_{n}\right)_{n \in \mathbb{N}}$ is not bounded, then the sequence does not converge.

We sometimes also say that in such a case the limit does not exist. We can use this to show that certain sequences do not converge.

Theorem 2.2.12. For all $p>0$, the sequence $\left(n^{p}\right)_{n \in \mathbb{N}}$ does not converge.

Proof. We first show that the sequence $\left(n^{p}\right)_{n \in \mathbb{N}}$ is unbounded. Let $p>0$. We need to show that

$$
\forall M \in \mathbb{R}: \exists n \in \mathbb{N}:\left|a_{n}\right|>M
$$

Let $M \in \mathbb{R}$. If $M \leq 0$, choose $n=1$, since then $a_{1}=1>0 \geq M$. If $M>0$, choose $n>M^{\frac{1}{p}}$, since then

$$
n^{p}>\left(M^{\frac{1}{p}}\right)^{p}=M .
$$

This proves that the sequence is unbounded and hence it follows from Corollary 2.2.11 that the sequence does not converge.

Example 2.2.13. We show that the sequence $(\log n)_{n \in \mathbb{N}}$ does not converge, by showing that it is unbounded.

Let $M \in \mathbb{R}$. If $M<0$, choose $n=1$, since then $\left|a_{1}\right| \geq 0>M$. If $M \geq 0$, then we want to find an $n \in \mathbb{N}$ such that $\log n>M$. We can choose $n \geq e^{M+1}$, so that this holds.
