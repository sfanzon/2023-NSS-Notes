...
$$
\mathcal{P}(\Omega) := \{ A \divider A \subset \Omega \}\,,
$$
...
\Omega = \{x_1, \ldots, x_m\}
$$
for some $m \in \N$. Then $\mathcal{P}(\Omega)$ contains $2^m$ elements. This is because for each $x_i \in \Omega$ we have just two choices: either include $x_i$ in a subset, or do not include $x_i$ in a subset.
:::
...
$$
R:=\{ (A,B) \in \mathcal{P}(\Omega) \times \mathcal{P}(\Omega) \, \colon \, A \subset B  \} \,.
$$
Let us check that $R$ is an **order relation**:

1. Reflexive: It holds, since $A \subset A$ for all $A \in \mathcal{P}(\Omega)$,

2. Transitive: If $A \subset B$ and $B \subset C$ then by definition of inclusion $A \leq C$.

3. Antisymmetric: If $A \subset B$ and $B \subset A$, then $A=B$ by Proposition \ref{Proposition-Equality-Sets}.
...
Assume given two sets $A$ and $B$. A **function** from $A$ to $B$ is a rule which associates **at each** element $x \in A$ a **single** element $y \in B$. Notations:

- We write $f \ \colon A \to B$ to indicate such rule,
- For $x \in A$, we denote by $f(x)$ the element $y \in B$ associated with $x$ by $f$.
...
$$
\{ y \in B \divider y = f(x) \, \mbox{ for some }\, x \in A \} \subset B \,.
$$
:::
...
We can use the definition of abosulte value to define the **absolute value function**. This is a map 
$$
...
In this section we carry out the proof of a seemingly trivial statement, to get used to the process. In a proof one needs to show that 
$$
\alpha \implies \beta
$${#eq-implication}
where 
...
To show (@eq-implication) we need to convince ourselves that $\beta$ follows by assuming $\alpha$. 

Common strategies to prove (@eq-implication) are:
...
$$
where $\neg \beta$ is the **negation** of $\beta$. For example we already proved by contradiction that 
$$
\mbox{Definition of } \Q  \, \implies \, \sqrt{2} \notin \Q \,,
$$
In the above statement 
$$
\alpha \leadsto \mbox{Definition of } \Q \,.
$$
$$
\beta  \leadsto \sqrt{2} \notin \Q \,.
$$
Therefore
$$
\neg \beta  \leadsto \sqrt{2} \in \Q \,.
$$ 

- **Direct**: Sometimes proofs will also need **direct** arguments, meaning that one need to show directly that (@eq-implication).

- **Contrapositive**: The statement  
is equivalent to (@eq-implication)
$$
\neg \beta \, \implies \, \neg \alpha \,.
$$ {#eq-contrapositive}
Thus, instead of proving (@eq-implication) one could instead show (@eq-contrapositive). The statement (@eq-contrapositive) is called the **contrapositive** of (@eq-implication).
...
a=b \,.
$$
We want to see that $\beta$ holds. Therefore fix an arbitrary $\e>0$. This means that $\e$ can be **any** positive number. Clearly
$$
|a-b| = |0| = 0 < \e
$$
...
Let $S$ be a subset of $\N$. Suppose that 
...
x_{n+1}:= \frac{x_n}{2} + 1 \,.
$$
A sequence defined as above is called **recurrence sequence**. Using the above rule we can compute all the terms of $x_n$: for example
$$
...
$$
By computing these terms we notice that the sequence might be increasing. Indeed we can prove by induction that 
$$
x_{n+1} \geq x_n
...
x_{n+2} \geq x_{n+1} \,.
$$
Indeed, we can multiply the inequality $x_{n+1} \geq x_n$ by $1/2$ and add $1$ to get
$$
\frac{x_{n+1}}{2} + 1 \geq \frac{x_{n}}{2} + 1   \,.
$$
The above is equivalent, by definition, to $x_{n+2} \geq x_{n+1}$.
